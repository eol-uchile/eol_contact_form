...
create_translations_catalogs: ## Create the initial configuration of .mo files for translation
	pybabel extract -F eol_contact_form/locale/babel.cfg -o  eol_contact_form/locale/django.pot --msgid-bugs-address=eol-ayuda@uchile.cl --copyright-holder=EOL *
	for lang in $(lang_targets) ; do \
        pybabel init -i eol_contact_form/locale/django.pot -D django -d eol_contact_form/locale/ -l $$lang ; \
        pybabel init -i eol_contact_form/locale/django.pot -D djangojs -d eol_contact_form/locale/ -l $$lang ; \
    done


update_translations: ## update strings to be translated
	pybabel extract -F eol_contact_form/locale/babel.cfg -o eol_contact_form/locale/django.pot *
	pybabel update -N -D django -i eol_contact_form/locale/django.pot -d eol_contact_form/locale/
	pybabel update -N -D djangojs -i eol_contact_form/locale/django.pot -d eol_contact_form/locale/
